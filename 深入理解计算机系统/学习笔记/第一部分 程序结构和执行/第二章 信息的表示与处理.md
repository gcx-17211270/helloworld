...
<h2>2.2 整数运算</h2>
<p class="indent">在本节中，我们要使用位来编码整数的两种不同的方式：一种只能表示非负数，而另一种能够用来表示负数、零和正数。后面我们将看到他们的数学属性和机器级实现方面密切关联。我们也会研究扩展或收缩一个已编码整数以适应不同长度表示的效果。</p>
<h3>2.2.1 整型数据类型</h3>
<h3>2.2.2 无符号数的编码</h3>
...
<p class="indent">2002年，从事FreeBSD开源操作系统项目的程序员意识到，他们对getpeername函数的实现存在安全漏洞，代码的简化版本如下</p>
</body>
</html>
...
<body>
    <p class="indent">在这段代码中，第7行给出的是库函数memcpy的原型，这个函数是要把一段指定长度为n字节从存储器的一个区域复制到另一个区域。从第14行开始的函数copy_from_kernel是要将操作系统内核维护的数据复制到指定的用户可以访问的存储器区域。对用户来说，大多数内核维护的数据结构应该是不可读的，因为这些数据结构可能包含其他用户和系统上运行的其他作业的敏感信息，但是显示为kbuf的区域是用户可以读的。参数maxlen给出的是分配给用户的缓冲区的长度，这个缓冲区是用参数user_dest指示的。然后，第16行的计算确保复制的字节数据不会超出源或者目标缓冲区可用的范围。</p>
    ...
    <p class="indent">我们可以看到，这个问题是由于数据类型不匹配造成的：在一个地方，长度参数是有符号数，而在另一个地方，它又是无符号数。正如这个例子所表明的那样，这样的不匹配会造成缺陷的原因，甚至会导致安全漏洞。幸运的是，还没有案例报告由程序员在FreeBSD上利用这个漏洞，他们发布了一个安全建议，"FreeNSD-SA-02:38.signed-error"，建议系统管理员如何应用补丁消除这个漏洞，要修正这个缺陷，只需要将copy_from_kernel的参数maxlen声明为类型size_t，与memcpy的参数n一致。同时，我们也应该将本地变量len和返回值声明为size_t。</p>
    <p class="indent">在使用无符号数的时候，尤其是涉及无符号数与有符号数的转换时候，会出现许多可能的错误和漏洞，正因如此，除了C语言外，很少语言支持无符号数，例如，Java只支持有符号数。设计者认为，无符号数带来的麻烦比收益多出太多。但即便如此，当我们仅仅将数字看作位的集合，而不是任何数学意义，无符号数仍然是有用的，例如一组各类标记(flag)的集合，在或者地址这类本来就无符号的数。对系统程序员来说，无符号数仍然是有用的。</p>
</body>
</html>
...
<body>
    <h2>2.3 整数运算</h2>
    <h3>2.3.1 无符号加法</h3>
    <p class="indent">无符号加法涉及的一个问题是溢出，例如，我们使用一个4位的符号数，他们的和可能需要5位才能表示([1111]<sub>2</sub>+[1111]<sub>2</sub>=[11110]<sub>2</sub>,但是对于只有4位的存储单元，高位的1将被舍弃，从而显示出的结果是[1110]<sub>2</sub>，并且将溢出位置1。可以看出，对于w位的数据运算时，发生了溢出时，得到的结果相当于正确结果减去2<sup>w</sup>。即，如果我们使用x、y来表示两个w位的数据，s表示计算观察到的结果，则有s=x+y(未溢出)、s=x+y-2<sup>w</sup>(溢出)。</p>
    <p class="indent">模数加法形成了一种数学结构，称为阿贝尔群(Abelian group)，这是以丹麦数学家Nieles Henrik Abel(1802~1829)的名字命名的。也就是说，它是可交换的和可可结合的。他有一个单位元0，并且每个元素都有一个加法逆元。</p>
    <h3>2.3.2 补码加法</h3>
    <p class="indent">两个数的w位补码之和和无符号之和有完全相同的位级表示。实际上，大多数计算机使用同样的机器指令来执行无符号或者有符号加法。补码的和会出现负溢出以及正溢出。(negative/positive overflow)</p>
    <h3>2.3.3 补码的非</h3>
    <p class="indent">补码的非运算(negation operation)定义如下</p>
    <ur style="text-indent:2em">
    <li>如果x=-2<sup>w-1</sup>,那么 非x=-2<sup>w-1</sup></li>
    <li>对于其他值，非运算等于-x</li>
    </ur>
    <p class="indent">在C语言中，我们可以确定，对于任意整数值x,计算表达式-x和~x+1得到的结果一样</p>
    <h3>2.3.4 无符号乘法</h3>
    <p class="indent">x*y&nbsp;=&nbsp;(x*y)&nbsp;mod&nbsp;2<sup>w</sup></p>
    <h3>2.3.5 补码乘法</h3>
    <p class="indent">C语言中的有符号乘法是通过将2w位的乘积截断为w位的方式实现的。我们认为对于无符号和补码来说，乘法运算的位级表示都是一样的。也就是，给定长度为w的位向量x和y，无符号乘积的x*y和补码乘积x*y的位级表示是相同的。这样，机器就可以使用一种乘法指令来进行有符号和无符号整数的乘法。</p>
    <p class="indent"><i>XDR库中的安全漏洞</i></p>
    <p class="indent">2002年，人们发现Sun Microsystems公司提供的实现XDR库的代码有安全漏洞，XDR库是一个广泛使用的程序间共享数据结构的工具，造成这个安全漏洞的原因是程序会在毫无察觉的情况下产生乘法溢出</p>
</body>
</html>
...
<head>
    ...
    <p class="indent">函数copy_elements的设计可以将ele_cnt个数据结构复制到第10行的函数分配的缓冲区中，每个数据结构包含ele_size个字节。需要的字节数是通过计算ele_cnt*ele_size得到的。</p>
    <p class="indent">如果一个怀有恶意的程序员用参数ele_cnt=1048577(2<sup>20</sup>+1),ele_size=4096(2<sup>12</sup>)来调用这个函数，然后第10行的乘法将溢出，导致只会分配4096个字节，而不是装下全部数据的字节。从第16行起的循环会使复制超过已分配的缓冲区，进而破坏其他的数据结构。这会导致程序崩溃或者行为异常。</p>
    <h3>2.3.6 乘以常数</h3>
    <p class="indent">在大多数机器上，整数乘法指令都相当慢，需要十个或者更多的时钟周期，然而其他整数运算（加法、减法、位级运算和移位）都只需要一个时钟周期。因而，编译器使用了一项重要的的优化，试着用移位和加法运算的组合来代替乘以常数因子的乘法。</p>
    <h3>2.3.7 除以2的幂</h3>
    <p class="indent">在大多数机器上，整数除法指令比整数乘法指令更慢，需要30个或更多的时钟周期，除以2的幂也可以用移位运算来实现，只不过我们实现的是右移而不是左移。</p>
    <p class="indent">在实现移位计算除以2的幂时候，对于正整数，例如77，我们计算77/2会得到38这样的结果，是因为小数部分被舍弃了，如果我们将这样的结果视为“正常”，那么对于负整数我们就会得到-77/2=-39这样的结果，因为我们在舍弃掉小数部分之后，我们是从“另一个方向”丢失了小数部分，从而得到了反常规的结果。利用偏置(biasing)技术可以帮助修正。</p>
    <p class="indent">当我们计算一个x和y(y&ne;0)的结果，有x/y=(x+y-1)/y。这当然是对于会自动舍弃小数部分的运算来说，因而，我们就可以利用这样的结果，在移位之前给出“y-1”的偏置，从而修正，用C语言的表达式写出来就是<code>(x&nbsp;&lt;&nbsp;0&nbsp;?&nbsp;(x&nbsp;+&nbsp;(1&nbsp;&lt;&lt;&nbsp;k&nbsp;-&nbsp;1)&nbsp;:&nbsp;x)&nbsp;&gt;&gt;&nbsp;k)</code><p>
    <h2>2.4 浮点数</h2>
    <p class="indent"><b>IEEE标准754</b>。在本节中，我们将会看到IEEE浮点数是如何表示数字的。我们还将探讨舍入(rounding)的问题，当一个数字无法被准确地表示为这种格式时，就必须向上调整或者向下调整，此时出现了舍入。</p>
    <h3>2.4.1 二进制小数</h3>
    <h3>2.4.2 IEEE浮点表示</h3>
    <p class="indent">IEEE浮点标准使用V=（-1）<sup>s</sup>*M*2<sup>E</sup>的形式来表示一个数。</p>
    <p class="indent">符号(sign)S。尾数(sifnificand)M。阶码(exponent)E。根据阶码和尾数的数值，我们可以将一个数分为：1.规格化的值（阶码非0非255，尾数任意。阶码字段被解释为以偏置形式表示的有符号整数，E=e-Bias,Bias=2<sup>k-1</sup>。小鼠字段是隐含的以1开头的。）2.非规格化的值（阶码为全0，尾数任意。此时阶码值被解释为E=1-Bias。尾数M=f（不再包含隐含开头的1））3.特殊值（阶码全1时的值，当小数域全0表示无穷，根据符号位1或0决定是负无穷或者是正无穷。当小数域非0则被称为"NaN"(Not a number)。一些运算的结果不能是实数或无穷，就会返回这样的结果。在某些应用中，表示未初始化的数据时，他们也很有用处。）</p>
</head>
</html>
